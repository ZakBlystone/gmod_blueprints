	...
	self.pins:AddListener(function(cb, action, id, var)
		...
		if self.module then
			if cb == bplist.CB_PREMODIFY then
				self.module:PreModifyNodeType( "__Make" .. self.id )
				self.module:PreModifyNodeType( "__Break" .. self.id )
			elseif cb == bplist.CB_POSTMODIFY then
				self.module:PostModifyNodeType( "__Make" .. self.id )
				self.module:PostModifyNodeType( "__Break" .. self.id )
			end
		end
